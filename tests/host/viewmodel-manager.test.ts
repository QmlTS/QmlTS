...
describe.skipIf(!isNativeModuleAvailable)('host/viewmodel-manager', () => {
	...
	beforeAll(async () => {
		...
		const vmMod = await import(
			'../../native/npm/qmlts-host/src/viewmodel-manager.ts'
		);
	...
	test('TV-04: sync() sends non-deferred state to host', () => {
		...
		expect(host.getProperty('LoginViewModel', 'isLoading')).toBe(
			'true',
		);
...
describe('host/viewmodel-manager (skip check)', () => {
	test('check native module availability', () => {
		if (!isNativeModuleAvailable) {
			console.log(
				'⚠️  Native module not built. Skipping ViewModelManager tests.',
			);
		}
