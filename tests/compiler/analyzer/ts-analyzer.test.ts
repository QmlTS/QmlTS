...
describe('TsAnalyzer', () => {
  describe('import discovery', () => {
    test('AN-03: analyzeSource with imports — imports populated correctly', () => {
      ...
      expect(result.imports[0]!.moduleSpecifier).toBe('../../dsl/generated/QtQuick/Rectangle.js');
      expect(result.imports[0]!.namedImports).toEqual(['Rectangle']);
      expect(result.imports[1]!.moduleSpecifier).toBe('../../viewmodel/index.js');
  ...
  describe('analyzeFile', () => {
    test('analyzeFile reads a real file', () => {
      const analyzer = createTsAnalyzer();
      const result = analyzer.analyzeFile(join(import.meta.dir, 'fixtures/simple-view.ts'));
  ...
  describe('analyzeProject', () => {
    test('AN-08: multi-file project — cross-file aggregation', () => {
      const analyzer = createTsAnalyzer();
      const tsconfigPath = join(import.meta.dir, 'fixtures/project/tsconfig.json');
