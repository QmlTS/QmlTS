...
class TsAnalyzerImpl implements TsAnalyzer {
  ...
  analyzeSource(source: string, fileName?: string): DiscoveredSourceFile {
    const virtualPath = fileName ?? `__qmlts_virtual__/source-${this.virtualCounter++}.ts`;
  ...
  private analyzeSourceFile(sf: import('ts-morph').SourceFile): DiscoveredSourceFile {
